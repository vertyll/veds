...
import com.vertyll.veds.template.application.saga.port.SagaStepRepository
...
internal class SagaStepPersistenceAdapter(
    ...
    override fun findBySagaIdAndStepNameAndStatus(
        ...
        repository.findBySagaIdAndStepNameAndStatus(sagaId, stepName, status).map {
            it.toDomain()
        }

    override fun findBySagaIdOrderByCreatedAtDesc(sagaId: String): List<SagaStep> =
        repository.findBySagaIdOrderByCreatedAtDesc(sagaId).map {
            it.toDomain()
        }
}
